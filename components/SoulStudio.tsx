...
export default function SoulStudio(){
 ...
 const [messages,setMessages]=useState<ChatMessage[]>([]),[draft,setDraft]=useState(''),[chatBusy,setChatBusy]=useState(false),[conversationId,setConversationId]=useState<string|null>(null),[matureEnabled,setMatureEnabled]=useState(false);
 ...
 const loadSaved=async(u:User)=>{const [{data},settings]=await Promise.all([supabase.from('characters').select('id,name,animal,prefix,suffix,visual_prompt,is_favorite,bio,personality,greeting,relationship_style,age_rating,visibility,voice_id,avatar_url').eq('user_id',u.id).order('created_at',{ascending:false}).limit(50),supabase.from('companion_settings').select('adult_enabled,mature_enabled').eq('user_id',u.id).maybeSingle()]);setSaved((data||[]) as Character[]);setMatureEnabled(Boolean(settings.data?.mature_enabled));};
 ...
 async function openChat(c:Character){setActive(c);setError('');setMessage('');setConversationId(null);setMessages([{role:'assistant',content:c.greeting||`Hey, I'm ${c.name}. ✨ What kind of world are we creating today?`}]);setTab('chat');if(user&&c.id){const {data}=await supabase.from('conversations').select('id').eq('user_id',user.id).eq('character_id',c.id).order('updated_at',{ascending:false}).limit(1).maybeSingle();if(data?.id){setConversationId(data.id);const {data:rows}=await supabase.from('messages').select('role,content').eq('conversation_id',data.id).order('created_at',{ascending:false}).limit(40);if(rows?.length)setMessages(rows.reverse() as ChatMessage[]);}}}
 async function sendMessage(){const text=draft.trim();if(!text||!active||chatBusy)return;const next=[...messages,{role:'user' as const,content:text}];setMessages(next);setDraft('');setChatBusy(true);setError('');try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json',...(user ? {Authorization:`Bearer ${(await supabase.auth.getSession()).data.session?.access_token||''}`} : {})},body:JSON.stringify({character:active,messages:next,conversationId,matureEnabled})});const d=await r.json();if(!r.ok)throw Error(d.error||'Chat failed.');setMessages([...next,{role:'assistant',content:d.reply}]);if(d.conversationId)setConversationId(d.conversationId)}catch(e){setError(e instanceof Error?e.message:'Chat failed.')}finally{setChatBusy(false)}}
 async function saveCreated(){if(!user||!createName.trim())return;setError('');const c={user_id:user.id,name:createName.trim(),bio:createBio.trim(),personality:createPersonality.trim()||'warm, friendly and curious',greeting:`Hi, I'm ${createName.trim()}. I'm glad you're here.`,relationship_style:'friendship',age_rating:'18+',visibility:'private',animal:'Original',prefix:'',suffix:'',visual_prompt:'Original AI companion portrait, expressive face, cinematic soft lighting',status:'draft',is_favorite:false};const {data,error:e}=await supabase.from('characters').insert(c).select('id,name,animal,prefix,suffix,visual_prompt,is_favorite,bio,personality,greeting,relationship_style,age_rating,visibility,voice_id,avatar_url').single();if(e){setError(e.message);return}if(data){setSaved(x=>[data as Character,...x]);openChat({...c,...data} as Character);setCreateName('');setCreateBio('');setCreatePersonality('');setMessage('Your AI companion is ready.')}}
 ...
 return <main className="soul-shell">
  ...
  <div className="soul-layout">
   ...
   <section className="soul-main">
    ...
    {tab==='profile'&&<div className="profile-page"><div className="profile-cover"><div className="profile-avatar">{user.email?.[0]?.toUpperCase()||'U'}</div></div><h1>{user.email?.split('@')[0]||'Soul User'}</h1><p>{user.email}</p><div className="profile-stats"><div><b>{saved.length}</b><span>Souls</span></div><div><b>{messages.length}</b><span>Recent messages</span></div><div><b>∞</b><span>Possibilities</span></div></div><div className="profile-setting"><div><b>18+ mature conversation</b><small>Allows romantic and mature non-graphic chat only.</small></div><button className={matureEnabled?'chosen':''} onClick={async()=>{const next=!matureEnabled;setMatureEnabled(next);await supabase.from('companion_settings').upsert({user_id:user.id,adult_enabled:next,mature_enabled:next,onboarding_complete:true})}>{matureEnabled?'Enabled':'Enable'}</button></div><button className="profile-signout" onClick={()=>supabase.auth.signOut()}>Sign out</button></div>}
