...
export default function BrainrotStudio(){
 ...
 const filtered=useMemo(()=>all.filter(c=>(category==='All'||c.animal===category||category==='Popular')&&(!search||c.name.toLowerCase().includes(search.toLowerCase())||(c.animal||'').toLowerCase().includes(search.toLowerCase()))),[all,search,category]);
